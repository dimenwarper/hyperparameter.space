...
*Left: Illustration of instrumental variables and the backdoor criterion. U is an unstrumental variable of X -> Y, while Z blocks all backdoor paths from X to Y, making it a confounder that can be leveraged to estimate the intervention effects of X on Y using observational data. Middle: the classic frontdoor criterion where Z is the "front door" from X to Y, allowing for an adjustment to estimate the effects of X on Y even if a confounder, U, is unobserved. Right: An example where no criterion can be used to calculate the effects of X on Y as there are backdoor paths blocked by an unobserved variable V, there are no instrumental variables, and no variables that act as front doors.*
...
<img src="/img/causal_models/drug_disease.png" width=50%>
...
<img src="/img/causal_models/drug_disease_collapsed.png" width=30%>
